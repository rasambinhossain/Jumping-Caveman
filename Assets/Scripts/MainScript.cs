...
public class MainScript : MonoBehaviour
{
    ...
    //[Tooltip("Will the system rely on config file for assistance input?")]
    //public bool assistanceFromConfig = false;

    //[Tooltip("Should it use Random Threshold Method?")]
    //public bool randomThresholdMethod = false;
    ...
    //public enum magnetismArea                       // enum following the input file to activate the magnetism area based on that  
    //{
    //    low = 1,
    //    medium  = 2,
    //    hard
    //}
    //public magnetismArea type = new magnetismArea();
    ...
    void placePole()
    {
        ...
        */
        ...
        //Debug.Log(textFileLines.Length);
        ...
        //string path = Application.dataPath + @"/" + file.Replace("file://", "");
        //Debug.Log(path);
        //using (var rd = new System.IO.StreamReader(path))
        //{
        //    while (!rd.EndOfStream)
        //    {
        //        var splits = rd.ReadLine().Split(',');
        //        columnX.Add(splits[0]);
        //        columnY.Add(splits[1]);
        //        columnClass.Add(splits[3]);
        //    }
        //    var counter = columnX.Count;
        ...
        foreach (GameObject pole in poleFrictionObjects)
        {
            ...
            switch(frictionValue.ToString())
            {
                case "0.1":
                    pole.GetComponent<BoxCollider2D>().sharedMaterial = (PhysicsMaterial2D)Resources.Load("Material/0.10");
                    break;
                case "0.15":
                    pole.GetComponent<BoxCollider2D>().sharedMaterial = (PhysicsMaterial2D)Resources.Load("Material/0.15");
                    break;
                case "0.2":
                    pole.GetComponent<BoxCollider2D>().sharedMaterial = (PhysicsMaterial2D)Resources.Load("Material/0.20");
                    break;
                case "0.25":
                    pole.GetComponent<BoxCollider2D>().sharedMaterial = (PhysicsMaterial2D)Resources.Load("Material/0.25");
                    break;
                case "0.3":
                    pole.GetComponent<BoxCollider2D>().sharedMaterial = (PhysicsMaterial2D)Resources.Load("Material/0.30");
                    break;
                case "0.35":
                    pole.GetComponent<BoxCollider2D>().sharedMaterial = (PhysicsMaterial2D)Resources.Load("Material/0.35");
                    break;
                case "0.4":
                    pole.GetComponent<BoxCollider2D>().sharedMaterial = (PhysicsMaterial2D)Resources.Load("Material/0.40");
                    break;
                case "0.45":
                    pole.GetComponent<BoxCollider2D>().sharedMaterial = (PhysicsMaterial2D)Resources.Load("Material/0.45");
                    break;
                case "0.5":
                    pole.GetComponent<BoxCollider2D>().sharedMaterial = (PhysicsMaterial2D)Resources.Load("Material/0.50");
                    break;
                case "0.55":
                    pole.GetComponent<BoxCollider2D>().sharedMaterial = (PhysicsMaterial2D)Resources.Load("Material/0.55");
                    break;
                case "0.6":
                    pole.GetComponent<BoxCollider2D>().sharedMaterial = (PhysicsMaterial2D)Resources.Load("Material/0.60");
                    break;
                case "0.65":
                    pole.GetComponent<BoxCollider2D>().sharedMaterial = (PhysicsMaterial2D)Resources.Load("Material/0.65");
                    break;
                case "0.7":
                    pole.GetComponent<BoxCollider2D>().sharedMaterial = (PhysicsMaterial2D)Resources.Load("Material/0.70");
                    break;
                case "0.75":
                    pole.GetComponent<BoxCollider2D>().sharedMaterial = (PhysicsMaterial2D)Resources.Load("Material/0.75");
                    break;
                case "0.8":
                    pole.GetComponent<BoxCollider2D>().sharedMaterial = (PhysicsMaterial2D)Resources.Load("Material/0.80");
                    break;
                case "0.85":
                    pole.GetComponent<BoxCollider2D>().sharedMaterial = (PhysicsMaterial2D)Resources.Load("Material/0.85");
                    break;
                case "0.9":
                    pole.GetComponent<BoxCollider2D>().sharedMaterial = (PhysicsMaterial2D)Resources.Load("Material/0.90");
                    break;
                case "0.95":
                    pole.GetComponent<BoxCollider2D>().sharedMaterial = (PhysicsMaterial2D)Resources.Load("Material/0.95");
                    break;
                case "1":
                    pole.GetComponent<BoxCollider2D>().sharedMaterial = (PhysicsMaterial2D)Resources.Load("Material/1.0");
                    break;
                default:
                    break;
            }
    ...
    void Update()
    {
        ...
        // This trail renderer's have been depreciated due to the projectory implementation
        // DEPRECIATED
        //if (realTrail)
        //    ninjaScript.realTrailVisibile = true;
        //else
        //    ninjaScript.realTrailVisibile = false;
        //if (playerTrail)
        //    ninjaScript.playerTrailVisible = true;
        //else
        //    ninjaScript.playerTrailVisible = false;
