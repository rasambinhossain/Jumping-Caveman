...
public class NinjaScript : MonoBehaviour
{
    ...
    void Jump(float jumpForce)
    {
        ...
        //Debug.Log("distanceOfNearestPoleXPosition: " + distanceOfNearestPoleXPosition);
        ...
        foreach (GameObject pole in poleMagObjects)
        {
            ...
            if (counter == finalCounter)            // turn on assistance only on the pole the player is go
            {
                ...
                else if(mainScript.TrajectoryAssistanceOn && mainScript.assistanceType == MainScript.TypeOfAssitance.randomThresholdMethod)
                {
                    ...
                    if (mainScript.difficultyLevel == MainScript.Difficulty.LowTrajectory)
                    {
        ...
        if (trajectoryAssistanceOn)
        {
            ...
            float requiredVelocity = (float)(0.5 * gravity * deltaX * deltaX);
            ...
            //Debug.Log("Required Velocity: " + requiredVelocity);
            float diff_bet_velocity = Math.Abs(requiredVelocity - inputVelocity);
            //Debug.Log("Required percentage: " + (diff_bet_velocity * 100)/requiredVelocity);
    ...
    void OnCollisionEnter2D(Collision2D collider)
    {
        ...
        else if (collider.gameObject.tag == "Ground")
        {
            ...
            // Log in to files
            logger.loggerMethod(0, 0, 0, "null", "null", "null", score, "null", distanceOfNearestPoleXPosition, assistanceLevel, hindranceLevel);
        ...
        else if(collider.gameObject.name == "PoleEnd")
        {
            ...
            if (ray.isSpotted())
            {
                ...
                // Log 
                logger.loggerMethod(1, poleDistance_fl, poleHeight, "PoleEnd", "0", "0", score, "null", distanceOfNearestPoleXPosition, assistanceLevel, hindranceLevel);
    ...
    public IEnumerator WaitForPlayerToStop(float poleDistance, float poleHeight, string currentPoleColl, string currentPoleMagnetism, string currentPoleFriction, string currentPoleMagnetismArea)
    {
        ...
        if (ray.isSpotted())
        {
            ...
            logger.loggerMethod(1, poleDistance, poleHeight, currentPoleName, currentPoleMag, currentPoleFric, score, currentPoleMagAr, distanceOfNearestPoleXPosition, assistanceLevel, hindranceLevel);
        ...
        {
            logger.loggerMethod(0, poleDistance, poleHeight, currentPoleName, currentPoleMag, currentPoleFric, score, currentPoleMagAr, distanceOfNearestPoleXPosition, assistanceLevel, hindranceLevel);
        }
